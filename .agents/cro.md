...
- **Ebook CTA in hero (QW1):** implementato e mantenuto. Hero confermata nella versione "lead magnet" che raccoglie email (paragrafo ebook + bottone "Vai all'Ebook" → `/shop/ebooks/introduzione-alla-sceneggiatura/`). La hero resta intatta per non disturbare la raccolta email.
- **Percorso a pagamento in home (HIC3):** i link a laboratori/corsi e consulenze di editing NON stanno più nella hero (rimossi per scelta). È stata aggiunta una **CTA band a tutta larghezza subito dopo le recensioni** (`src/app/(home)/_components/services-cta.tsx`): sfondo scuro per marcare il momento "decision", due card (Editing Double View → `/shop/servizi-di-editing/`; Laboratori & Corsi → `/shop/corsi-di-sceneggiatura/`) con proof (7-10 gg, prezzo sotto media, ★5 su 7 recensioni). Il terzo blocco in "I nostri servizi" (con le stesse CTA) resta presente come percorso secondario in fondo.
- **Struttura home attuale:** Hero (lead magnet) → Reviews (trust) → CTA band (consideration/decision) → I nostri servizi (editoriale) → Ultimi articoli → Contatti.