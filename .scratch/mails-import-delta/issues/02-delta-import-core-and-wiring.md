...
- [ ] A provider error on a single contact is reported in the per-contact errors without aborting the rest of the import
- [ ] The import mutation is rewired to the new orchestration; its response stays compatible with the current UI (synchronous, message + counts)
- [ ] The now-dead full-import helper is removed (no remaining callers); the full audience re-sync mutation and the filtered-contact-count query are unchanged
- [ ] Core tests (fake adapter, existing test seam) cover: the delta query selects only missing contacts, empty delta → zero provider calls, persistence of created contacts' provider ids, per-contact errors do not block the rest, and local connection happens before the provider call