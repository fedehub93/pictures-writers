...
const fetchSubscriptionGrowth = async (
  from: string,
  to: string
): Promise<GrowthStats> => {
...
export const useSubscriptionStatsQuery = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["mailContactStats", from, to], // Chiave unica per la cache basata sulle date
    queryFn: () => fetchSubscriptionGrowth(from, to), // Funzione fetch
