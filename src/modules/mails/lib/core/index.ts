...
export async function propagateContactCreate(
  ...
  try {
    ...
    if (upsertResult.errors.length > 0) {
      return {
        propagationWarning: `Provider error: ${upsertResult.errors.join(", ")}`,
      };
    }
    ...
    console.error("[propagateContactCreate] Unexpected error:", error);
    return {
      propagationWarning:
...
export async function propagateContactUpdate(
  ...
  try {
    ...
    console.error("[propagateContactUpdate] Unexpected error:", error);
    return {
      propagationWarning:
        error instanceof Error ? error.message : "Unknown propagation error",
    };
