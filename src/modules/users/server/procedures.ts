...
const getQualifiedAdministratorCount = async (client: DbClient) => {
  const users = await client.user.findMany({
    where: {
      ...
      roleDefinition: { is: { isActive: true } },
    },
    select: {
      id: true,
      accountStatus: true,
...
const getActiveRole = async (client: DbClient, id: string) => {
  ...
  if (!role || !role.isActive) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Users can only be assigned an active role",
    });
  }
  return role;
};
...
export const usersRouter = createTRPCRouter({
        ...
        async (transaction) => {
          ...
          assertAccountChangeAllowed({
            ...
            qualifiedAdministratorCount:
              await getQualifiedAdministratorCount(transaction),
          ...
          await recordAdministrativeActivity(transaction, {
            ...
            before: {
              ...
              roleId: target.roleDefinition?.id ?? null,
            },
            after: {
              ...
              roleId: updated.roleDefinition?.id ?? null,
            },
          });
          return updated;
        },
  ...
  getInvitations: permissionProcedure(PERMISSIONS.USERS_READ).query(
    async () => {
  ...
  getActivityHistory: permissionProcedure(PERMISSIONS.USERS_READ)
    ...
    .query(({ input }) =>
      (
        db as typeof db & {
          administrativeActivity: Prisma.AdministrativeActivityDelegate;
        }
      ).administrativeActivity.findMany({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        await db.invitation.update({
          ...
          data: {
            status: "CANCELLED",
            tokenHash: `cancelled-${crypto.randomUUID()}`,
          },
        });
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        if (
          ...
          throw new Error("Email delivery is not configured");
        const current = await db.invitation.findUnique({
          where: { id: invitation.id },
  ...
  cancelInvitation: permissionProcedure(PERMISSIONS.USERS_MANAGE)
    ...
    .mutation(async ({ input, ctx }) => {
      const invitation = await db.invitation.findUnique({
        where: { id: input.id },
        select: { status: true, email: true, role: { select: { name: true } } },
      });
      if (!invitation || invitation.status !== "PENDING")
        throw new TRPCError({
          code: "BAD_REQUEST",
      ...
      const cancelledInvitation = await db.invitation.findUniqueOrThrow({
        where: { id: input.id },
      });
      await recordAdministrativeActivity(db, {
        actorId: ctx.auth.id,
        ...
        after: { status: cancelledInvitation.status },
      });
